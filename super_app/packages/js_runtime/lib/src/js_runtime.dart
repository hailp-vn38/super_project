...
class JsRuntime {
  ...
  Future<T> getDetail<T>({required String url, required String jsScript}) async {
    return _runExtension<T>(() async {
      _evaluateJsScript(jsScript);
      final jsResult = await _runtime.handlePromise(
  ...
  Future<T> getChapters<T>(
      {required String url, required String jsScript, int? page = 1}) async {
    return _runExtension<T>(() async {
      _evaluateJsScript(jsScript);
      final jsResult = await _runtime.handlePromise(
  ...
  Future<T> getChapter<T>({required String url, required String jsScript}) async {
    return _runExtension<T>(() async {
      _evaluateJsScript(jsScript);
      final jsResult = await _runtime.handlePromise(
  ...
  Future<T> getSearch<T>(
      ...
      int? page,
      required String jsScript}) async {
    return _runExtension<T>(() async {
      _evaluateJsScript(jsScript);
      final jsResult = await _runtime.handlePromise(
          await _evaluateAsyncJsScript(
  ...
  Future<T> testFun<T>({required String source}) async {
    return _runExtension<T>(() async {
      _evaluateJsScript(source);
      final jsResult = await _runtime
          .handlePromise(await _evaluateAsyncJsScript('stringify(()=>"test")'));
